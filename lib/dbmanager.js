...
class DBManager {
  ...
  async getDashboardData(week, year) {
    ...
    // Get data from both Bhaskar Node and Harshitha Node
    const bhaskarSnap = await db
      .collection("users")
      .doc("Bhaskar Node")
    ...
    if (bhaskarSnap.exists) {
      networking += parseInt(bhaskarSnap.data().networking || 0);
      infos += parseInt(bhaskarSnap.data().infos || 0) + parseInt(bhaskarSnap.data().reinfos || 0);
      invis += parseInt(bhaskarSnap.data().invis || 0);
      plans += parseInt(bhaskarSnap.data().plans || 0);
      count += parseInt(bhaskarSnap.data().nodeCount || 0);
    }

    if (harshithaSnap.exists) {
      networking += parseInt(harshithaSnap.data().networking || 0);
      infos += parseInt(harshithaSnap.data().infos || 0) + parseInt(harshithaSnap.data().reinfos || 0);
      invis += parseInt(harshithaSnap.data().invis || 0);
      plans += parseInt(harshithaSnap.data().plans || 0);
      count += parseInt(harshithaSnap.data().nodeCount || 0);
    }
  ...
  async getDashboardChartData(week, year) {
    ...
    for (let wkCount = 1; wkCount <= 10; wkCount++) {
      ...
      if (bhaskarSnap.exists) {
        uv += parseInt(bhaskarSnap.data().uV || 0);
        plans += parseInt(bhaskarSnap.data().plans || 0);
        count += parseInt(bhaskarSnap.data().nodeCount || 0);
        networking += parseInt(bhaskarSnap.data().networking || 0);
        infos += parseInt(bhaskarSnap.data().infos || 0) + parseInt(bhaskarSnap.data().reinfos || 0);
      ...
      if (harshithaSnap.exists) {
        uv += parseInt(harshithaSnap.data().uV || 0);
        plans += parseInt(harshithaSnap.data().plans || 0);
        count += parseInt(harshithaSnap.data().nodeCount || 0);
        networking += parseInt(harshithaSnap.data().networking || 0);
        infos += parseInt(harshithaSnap.data().infos || 0) + parseInt(harshithaSnap.data().reinfos || 0);
  ...
  // Remove old getDashboardData and getDashboardChartData methods
  async getDashboardDataOld(week, year) {
    var networking = 0;
    var infos = 0;
    var invis = 0;
    var plans = 0;
    ...
    for (let i = 0; i < snapshot.length; i++) {
      ...
      if (snap.exists) {
        networking += parseInt(snap.data().networking || 0);
        infos += parseInt(snap.data().infos || 0) + parseInt(snap.data().reinfos || 0);
        invis += parseInt(snap.data().invis || 0);
        plans += parseInt(snap.data().plans || 0);
        count += parseInt(snap.data().nodeCount || 0);
      }
    }
    return {
      networking: networking,
  ...
  async getDashboardChartDataOld(week, year) {
    var datas = [];
  ...
  async getCollectionData(group, year, week) {
    ...
    if (group == "Bhaskar Node") {
      collection = "users";
      docName = "Bhaskar Node";
    } else if (group == "Harshitha Node") {
      collection = "users";
      docName = "Harshitha Node";
    } else {
      collection = "users";
      docName = group;
    }
    ...
    if (snap.exists) {
      var dataRow = "{";
      dataRow += ' "sl" : "1",';
      dataRow += ' "name" : "' + docName + '",';
      var fields = await this.getFields(group);
      fields.push("remarks");

      for (let i = 0; i < fields.length; i++) {
        dataRow += '"' + fields[i] + '" : "' + (snap.data()[fields[i]] || 0) + '"';
  ...
  updateTotalToNode(week, year, obj, nodeName) {
  ...
  async addUser(name, group) {
    const fields = await this.getFields("SKB"); // Use SKB fields for all groups
  ...
  async getOnlyUserNames(group) {
    ...
    if (group == "Bhaskar Node") {
      docArray.push("Bhaskar Node");
    } else if (group == "Harshitha Node") {
      docArray.push("Harshitha Node");
    } else {
      const snapshot = await db.collection("users").listDocuments();
      for (let i = 0; i < snapshot.length; i++) {
  ...
  async getUserNamesSapphire() {
    const docArray = [];
    docArray.push({ name: "Bhaskar Node" });
    docArray.push({ name: "Harshitha Node" });
    return docArray;
  }
