...
function sumData(week, year, group) {
  for (let i = settingsJson.totalViewSKBColSpan; i < fields.length - 1; i++) {
    var total = 0;
    $(`.${fields[i]}_week${week}_${year}_${group.replace(' ', '')}`).each(function () {
      if ($(this).val() != "") {
        total += parseInt($(this).val());
      }
    });
    $(".total" + fields[i] + `_week${week}_${year}_${group.replace(' ', '')}`).html(total);
  }
}

function updateTotalToNode(wk, yr, group) {
  var data = "{";
  for (let i = 0; i < settingsJson.connections.length; i++) {
    const val = $(".total" + settingsJson.connections[i].startNode + `_week${wk}_${yr}_${group.replace(' ', '')}`).html();
  ...
  var updateData = { week: wk, year: yr, obj: JSON.parse(data), nodeName: group };
  const xhttp = new XMLHttpRequest();
  xhttp.open("POST", "/view/updateTotalToNode");
...
function valueChanged(wk, yr, docName, triggeredFrom, group = "Bhaskar Node") {

  if (group == "Bhaskar Node" || group == "Harshitha Node") {
    var value_input = $("." + docName + "-" + triggeredFrom + `_week${wk}_${yr}_${group.replace(' ', '')}`).val();
    ...
    xhttp.onload = function () {
      if (this.responseText == "success") {
        showAlert(group + " updated :  <strong>" + docName + "</strong> - " + triggeredFrom);
      } else {
      ...
      for (let j = 0; j < fields.length; j++) {
        if (j == 0) {
          ...
          rowTable += `<td class="bl br bb text-center px-1"><input type="text" class="input input-sm ${getTDClass(
            fields[j]
          )} ${fields[j]} ${jsonData[i].name + "-" + fields[j] + `_week${week}_${year}_${group.replace(' ', '')}`}" value="${jsonData[i][fields[j]]
            }" onchange="valueChanged('${week}','${year}','${jsonData[i].name}', '${fields[j]
            }', '${group}')"></td>`;
        } else {
          rowTable += `<td class="bl br bb text-center px-1"><input type="text" class="input input-sm ${getTDClass(
            fields[j]
          )} ${fields[j]}_week${week}_${year}_${group.replace(' ', '')} ${jsonData[i].name + "-" + fields[j] + `_week${week}_${year}_${group.replace(' ', '')}`
    ...
    for (let i = settingsJson.totalViewSKBColSpan; i < fields.length; i++) {
      if (i == fields.length - 1) {
        ...
        rowTable += `<th class="bl br bb text-center ${getTDClass(
          fields[i]
        )} total${fields[i]}_week${week}_${year}_${group.replace(' ', '')}"></th>`;
      }
    }
    rowTable += "</tr>";
    tab_content += rowTable;
  } else {
    // Handle as SKB format for other groups
    // Same logic as above
  }
                        ...
                        <div class="tab-content bg-base-100 border-base-300 p-6">
							<div class="overflow-x-auto">
								<table id="table_week${week}_${group}" class="table table-xs table-zebra">
									<thead>
										${group == "Bhaskar Node" || group == "Harshitha Node"
      ? generateSKBTable(
...
function loadTabContent(week, year, group, jsonData) {
  ...
  if (group == "Bhaskar Node" || group == "Harshitha Node") {
    ...
    // Handle other groups same as SKB
  }

  $(`#table_week${week}_${group.replace(' ', '')}`).children("tbody").append(tab_content);
  $(`#loader_week${week}_${group.replace(' ', '')}`).addClass("hidden");
  $(`#loader_week${week}_${group.replace(' ', '')}`).siblings(".tab_icon").removeClass("hidden");
...
function loadHeader(week, year, group) {
  const tab = `
            <label class="tab text-start items-center">
              <input type="radio" name="my_tabs_4" checked />
              <i class="tab_icon size-4 me-2 hidden" data-lucide="calendar"></i>
              <div id="loader_week${week}_${group.replace(' ', '')}" class="loading loading-spinner loading-xs me-2"></div>
              <span class="flex mr-7 items-center">
                                Week 
                                <span class="week ml-1">${week}</span>, 
                                <span class="year">${year}</span> 
            ...
            <div class="tab-content bg-base-100 border-base-300 p-6">
							<div class="overflow-x-auto">
								<table id="table_week${week}_${group.replace(' ', '')}" class="table table-xs table-zebra">
									<thead>
										${group == "Bhaskar Node" || group == "Harshitha Node"
      ? generateSKBTable(
        settingsJson.initTableView.concat(
